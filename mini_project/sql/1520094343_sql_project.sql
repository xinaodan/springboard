...
SELECT members.firstname, members.surname, MAX( starttime ) 
...
SELECT booking_cost.name, CONCAT( members.firstname,  ' ', members.surname ) AS fullname, booking_cost.cost
FROM (
...
THEN facilities.guestcost * bookings.slots
ELSE facilities.membercost * bookings.slots
END AS cost, bookings.memid, bookings.starttime
FROM  `Bookings` bookings
LEFT JOIN  `Facilities` facilities ON bookings.facid = facilities.facid
)booking_cost
LEFT JOIN  `Members` members ON members.memid = booking_cost.memid
WHERE starttime LIKE  '2012-09-14%'
AND cost >30
ORDER BY cost DESC;
